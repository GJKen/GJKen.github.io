// ==UserScript==
// @author          GJK_en
// @name            各大翻译页面Dark模式
// @namespace       Telegram@GJK_en
// @icon            https://i.loli.net/2019/04/22/5cbd720718fdb.png
// @description	    精简页面|Dark模式|移除广告, 适配:百度翻译|谷歌翻译|有道翻译|金山词霸|必应翻译|搜狗翻译
// @version         0.0.1
// @include         *://fanyi.baidu.com/*
// @include         *://www.iciba.com/*
// @include         *://translate.google.com/*
// @include         *://fanyi.youdao.com/*
// @include         *://cn.bing.com/translator/*
// @include         *://fanyi.sogou.com/*
// @grant           GM_addStyle
// @run-at          document-start
// @supportURL      https://gist.github.com/Hunlongyu/fbd80bac47b8a2aba216fe5c3399b464
// ==/UserScript==

(function(){
	'use strict'
	const website = [
	{
		name: 'baidu',
		css: `
			.header, .footer, .trans-other-right, .manual-trans-btn{display: none !important;}
		`
	},
	{
		name: 'iciba',
		css: `
			html{background: #3988e5;}
			.home-banner{background-image: none !important;background: #3988e5;}
			.nav, .home-download, .home-func, .home-pc, .footer, .cb-downmask, .ad-left, .search-ad, .base-bt-bar{display: none !important;}
			.common-top, .container-right, .info-product, .info-hotwords, .goto-top-2, .foot-top-seo,.ad-left2, .foot{display: none !important;}
		`
	},
	{
		name: 'google',
		css: `
			.yUUmWd.VfPpkd-AznF2e-OWXEXe-auswjd .VfPpkd-jY41G-V67aGc, .JJYS0b:hover .VfPpkd-jY41G-V67aGc,
			.JJYS0b.VfPpkd-ksKsZd-mWPk3d-OWXEXe-AHe6Kc-XpnDCe .VfPpkd-jY41G-V67aGc, .JJYS0b:not(.VfPpkd-ksKsZd-mWPk3d):focus .VfPpkd-jY41G-V67aGc,
			.JJYS0b:active .VfPpkd-jY41G-V67aGc, .qSb8Pe.RCaXn:not(.KKjvXb) .S3Dwie{
				color: #ff0000;
			}
			.Rj2Mlf:hover:not(:disabled), .Rj2Mlf.VfPpkd-ksKsZd-mWPk3d-OWXEXe-AHe6Kc-XpnDCe:not(:disabled), .Rj2Mlf:not(.VfPpkd-ksKsZd-mWPk3d):focus:not(:disabled), .Rj2Mlf:active:not(:disabled),
			a.kmrl0b{
				color: #44b1ff;
			}
			.MZgjEb{color: #da3cd6;}
			.I87fLc.XzOhkf{border: none;}
			.usGWQd{background :#ffeddb;}
			.VhOj3e, .a88hkc{color: #f27474;}
			.Xcmxjb.FwR7Pc{background: #7b1f1f}
			.WdefRb{color: #5f6368 !important;}
			.VfPpkd-LgbsSe{background: #3d4554;}
			.X4DQ0::after{display: none !important;}
			.ita-kd-inputtools-div{background: #768db7;}
			.cfWmIb .VfPpkd-fmcmS-wGMbrd{color: #f4ff00;}
			.yUUmWd .VfPpkd-jY41G-V67aGc{color: #ffe66c;}
			.RvYhPd::before, .pEyuac{background: #1a202c;}
			.Pt010b .Sp3AF{border-right: 2px solid #fffb84;}
			.BL1djf, .LV4pme{color: #c2c53a; font-style: normal;}
			body, .gHl6Mc, .ita-kd-menuitem, .qSb8Pe{color: #c78846;}
			.app-download-bar, #gb, .ODAbH{display: none !important;}
			span.VfPpkd-vQzf8d, svg.AeYb4d.NMm5M, .mvqA2c{color: #bce7ff;}
			.PRdtG .VfPpkd-AznF2e-wEcVzc-OWXEXe-NowJzb{border-color: #ff0000;}
			.PsfnLc{color: #da5050;border: 2px solid rgba(251, 255, 0, 0.22);}
			.yUUmWd:focus.VfPpkd-AznF2e-OWXEXe-auswjd .VfPpkd-jY41G-V67aGc, .aia4Ud{color: #ff0000;}
			.ita-kd-dropdown-menu, .ita-kd-menuitem-hover, .ita-kd-inputtools-div{border-radius: 20px;}
			.er8xn, .UdTY9, .Qk6AXd, qSb8Pe, .qSb8Pe.KKjvXb, .JqKQbd div, .sp2Bqb span{color: #e2e2e2;}
			.ccvoYb, .RvYhPd, .rm1UF, .qSb8Pe:hover, .qSb8Pe:focus, .qSb8Pe.KKjvXb{background: #3d4554;}
			.SSgGrd:not(:disabled), .qiN4Vb:not(:disabled), .qiN4Vb:not(:disabled) .VfPpkd-kBDsod{color: #5da5ff;}
			.VfPpkd-NSFCdd-Ra9xwd span, .VfPpkd-NLUYnc-V67aGc, .VfPpkd-fmcmS-wGMbrd, .VfPpkd-fmcmS-TvZj5c-OWXEXe-UbuQg{color: #fff !important;}
			html, .Dwvecf, .jTj8gd.XzOhkf .a2Icud, .SMqeCb, .yFQBKb, .fMHXgc, .oLbzv, .FM9KDb, .EO28P, .ZShpvc, .GQpbTd.WZapbb{background: #2d3748;}
			.D4w5q, .eIKIse, .VhOj3e, .VK4HE:hover .dPxysd, .VK4HE:focus .dPxysd, .qTO0c:hover .dPxysd, .qTO0c:focus .dPxysd,.cfWmIb:hover:not(.VfPpkd-fmcmS-yrriRe-OWXEXe-OWB6Me) .VfPpkd-NLUYnc-V67aGc, .TcXXXb{
				color: #ffd868;
			}
			.nYkDR, .S6GkK, .dPxysd, .ZTPlmc, .RSggmb, .xex4Kc, .qSb8Pe:not(.KKjvXb) .qGfVtc, yFQBKb,.AarNVd, .cfWmIb.VfPpkd-fmcmS-yrriRe-OWXEXe-XpnDCe:not(.VfPpkd-fmcmS-yrriRe-OWXEXe-OWB6Me) .VfPpkd-NLUYnc-V67aGc,
			.VfPpkd-NSFCdd-Ra9xwd span, .VfPpkd-NLUYnc-V67aGc, .VfPpkd-fmcmS-wGMbrd, .VfPpkd-fmcmS-TvZj5c-OWXEXe-UbuQg{color: #fff !important;}{
				color: #fff;
			}
			.Rj2Mlf:not(:disabled),
			.cfWmIb.VfPpkd-fmcmS-yrriRe-OWXEXe-XpnDCe:not(.VfPpkd-fmcmS-yrriRe-OWXEXe-OWB6Me) .VfPpkd-NSFCdd-Brv4Fb, .cfWmIb.VfPpkd-fmcmS-yrriRe-OWXEXe-XpnDCe:not(.VfPpkd-fmcmS-yrriRe-OWXEXe-OWB6Me) .VfPpkd-NSFCdd-Ra9xwd, .cfWmIb.VfPpkd-fmcmS-yrriRe-OWXEXe-XpnDCe:not(.VfPpkd-fmcmS-yrriRe-OWXEXe-OWB6Me) .VfPpkd-NSFCdd-MpmGFe{
				border-color: #bce7ff;
			}
		`
	},
	{
		name: 'youdao',
		css: `
			body{background: #1a202c;}
			.doc__upload--con{color: #000;}
			.fanyi__footer{border-top:none;}
			.fanyi-error-message p{color: #fff;}
			.fanyi-error-message{font-size: 20px;}
			.doc__upload--con{background: #28f5ff;}
			.fanyi__update__tip{display:none !important;}
			.fanyi__nav__list li a:hover{color: #28f5ff;}
			.fanyi__input__bg, .input__target__text{background: #cba875;}
			.input__original__bar--fonts, .target__bar__update{color: #cc4949;}
			.fanyi__nav__list li a,.fanyi__operations--underline label,.item-select .select-text{color: #ff5e00;}
			.writing_new,.rengong__guide,.doc__upload--tip,.inside__products,.side__nav,.doc__upload--con .circle{
				display:none !important;
			}
			.fanyi__nav__logo{
				width: 120px;
				background: url(data:image/png;base64,) 50% 50% no-repeat !important;
			}
		`
	},
	{
		name: 'bing',
		css: `
			#tt_translatorHome{width: 96% !important;}
			.t_header, .t_navigation, .b_footer{display: none !important;}
		`
	},
	{
		name: 'sogou',
		css: `
			body{ min-height: inherit !important;}
			.header, .logo-translation, .trans-footer, .header-fanyi, #J_download_app, .trans-tab{display: none !important;}
		`
	}
	]

	let url = window.location.href
	for(let i = 0; i < website.length; i++){
		if(url.indexOf(website[i].name) !== -1){
			return GM_addStyle(website[i].css)
		}
	};
})()